...
echo.
echo Als Player werden benutzt:
echo - 3x bitcontrol(r) Simple Player
echo      fungiert gleichzeitig auch als Broadcaster und Streaming Server
echo - Adobe Flash Video Plugin im IE Browser
echo - VLC (Video LAN Client)
echo.
echo Die Verzoegerung der Bilddarstellung ist fuer bessere Anschaulichkeit des
echo Beispiels kuenstlich verlaengert.
echo.
...
REM es soll keine Fehlermeldung ausgeworfen werden, sondern zu eine Sprungmarke gesprungen werden, außer bei winver_unknown
...
:x264vfw-check
echo ueberpruefe x264vfw auf 64-Bit machine in %PF%
if not exist "%PF%\x264vfw" goto x264vfw-missing
echo ...OK: "%PF%\x264vfw" verzeichnis existiert
if not exist "%PF%\x264vfw\x264vfw.dll" goto x264-missing
echo ...OK: "%PF%\x264vfw\x264vfw.dll" existiert
